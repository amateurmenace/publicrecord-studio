...
| Piece | What it is |
|---|---|
| `web/` | The press: `bake.py` reads the corpus and `emit.py` writes the edition — every page, plane and stub the reader touches; `broadsheet.py` lays the front page out as a paper (specs/29 — the masthead, the search spine, tonight's tape with the score of the night, the year in tapes, four columns, the lens river); `story.py`, `charts.py` and `topic.py` write its words and draw its pictures (counted, never modeled; every picture carries its numbers beside it for the reader to re-light); `record/stills.py` presses each tape's poster and frames into the edition once; `gallery.py` lists readers' shared front pages from the share store beside the record's own (specs/29 P2 — cards judged by their receipts, nobody named); `pictures.py` presses every picture as a downloadable .svg; `glossary.py` explains the words the record uses, with their sources and the record's count of them. `web/static/app.js` is the whole client (no build step): reading, search told as a story, the reel composer and player, the studio, your paper. |
