...
| Piece | What it is |
|---|---|
| `web/` | The press: `bake.py` reads the corpus and `emit.py` writes the edition — every page, plane and stub the reader touches; `story.py`, `charts.py` and `topic.py` tell the front page's stories (the record over time, the latest meeting, a word over time), counted, never modeled; `pictures.py` presses every picture as a downloadable .svg; `glossary.py` explains the words the record uses, with their sources and the record's count of them. `web/static/app.js` is the whole client (no build step): reading, search told as a story, the reel composer and player, the studio, your paper. |
