...
class TestPaper(unittest.TestCase):
    ...
    def test_featured_papers_match_the_js_codec_byte_for_byte(self):
        ...
        body = "\n".join([
            ...
            "function fail(m){ console.log('FAIL', m); process.exit(1); }",
            "for (const qs0 of FEATS) {",
            "  // the press marks its own links by=press (decided 2026-09-24): the",
            "  // reader reads it as `by` and the codec twin holds on the rest",
            "  if (!/&by=press$/.test(qs0)) fail('no press marker on '+qs0);",
            "  const back = decodePaper('?' + qs0);",
            "  if (back.by !== 'press') fail('by not read from '+qs0);",
            "  const qs = qs0.replace(/&by=press$/, '');",
